...
object BCardModel {
  ...
  def noScore(ss: RDD[Tuple2[String, Iterable[CommonEvent]]]):RDD[Tuple4[String, String, String, String]] = {
    //    val noScoreKequn = getTable(ss,session).mapPartitions(item => {
    val noScoreKequn = getTable(ss).mapPartitions(item => {

      for (x <- item) yield {
        if (x._2 <= 0 && x._3 <= 0) {
          (x._1,null:String, "1", x._6)
        } else if (x._2 <= 0 && x._3 > 0 && x._4 <= 0 && x._5 <= 0.01) {
          (x._1,null:String, "2", x._6)
        } else if (x._2 <= 0 && x._3 > 0 && x._4 > 0 && x._5 <= 0.01) {
          (x._1,null:String, "3", x._6)
        } else {
          null
        }
      }
  ...
  def getTable(ss: RDD[Tuple2[String, Iterable[CommonEvent]]]) = {
    ...
    val rdd = ss.flatMap(x => {
      ...
      for (i <- x) yield {
        (i.MEMBER_CODE, i.ACCOUNT_NMBR, i.PYMT_FLAG, i.DELQ_STATUS, i.PRINCIPAL, i.TRAN_AMT_PAID,
          i.CASH_AMT, i.POSTING_DTE, i.STATUS, i.TRANSACTION_TYPE, i.PAYMENT_DTE,
          i.LST_UPD_TIME, i.AUDIT_TIME)
      })
      .toDF("MEMBER_CODE", "ACCOUNT_NMBR", "PYMT_FLAG", "DELQ_STATUS", "PRINCIPAL", "TRAN_AMT_PAID",
    ...
    val result = session.sql(
      """
        |select b.member_code,sum(b.principal)-sum(b.tran_amt_paid) as balance,
        ...
        |from(
        |select member_code,account_nmbr,principal,POSTING_DTE,sum(tran_amt_paid) as tran_amt_paid,
        |SUM(CASE WHEN DATEDIFF(NOW(), POSTING_DTE)<=365 THEN TRAN_AMT_PAID ELSE 0 END) AS OneYear,
